...
export async function connectSigner(
  ...
  const board = async () => {
    ...
    const c = createContract(runtime, WHENWE, WRITE_ABI, { defaultSigner: signer as never }) as Record<
      string,
      {
...
export async function openPoll(s: Signer, title: string, options: string[]): Promise<string> {
  const b = await s.board();
  return b.write('open', [s.mask, title, options]);
}

/** Tick the slots that suit you. Voting again replaces the previous answer. */
export async function castVote(s: Signer, id: number, bitmap: number): Promise<string> {
  const b = await s.board();
  return b.write('vote', [s.mask, BigInt(id), bitmap]);
}
