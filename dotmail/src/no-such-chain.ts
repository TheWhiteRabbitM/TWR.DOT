/**
 * A stand-in for the chain descriptors dotmail can never ask for.
 *
 * WHAT THIS SAVES. The published bundle was 1.5 MB, and 5.1 MB of it was chain
 ...
 * runs. dotmail passes `environment: 'devnet'` at its single call site
 * (src/chainstore.ts) and imports `devnet-asset-hub` directly everywhere else — it
...
const absent = (chain: string) =>
  new Proxy({} as Record<string, unknown>, {
    get: (_t, prop) => {
      ...
      throw new Error(
        `dotmail does not bundle the ${chain} metadata — something read "${String(prop)}" off it. ` +
