...
export async function runProbe(): Promise<Finding[]> {
  ...
  out.push(await entropyScope('dot-drive:probe:v1'));
  ...
  out.push(await foreignKey('dotmail.jmap', "another app's mail server credentials"));
  out.push(await foreignKey('dotmail.devseed', "another app's trial mailbox seed"));
...
export function asText(findings: Finding[]): string {
  ...
  return [
    'dot-drive sandbox probe',
    when,
    navigator.userAgent.slice(0, 110),
    '',
    ...lines,
    '',
    'Run the same probe in dotmail and compare the two dotmail:x25519:v1 public keys.',
  ].join('\n');
}
