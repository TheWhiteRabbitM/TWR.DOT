...
export async function publishKeyToMask(mask: number, keyHex: string): Promise<PublishResult> {
  try {
    ...
    if (!c.manager || !c.address) {
      return { ok: false, hostCannot: true, why: 'no account here to sign with' };
    }

    // Signed exactly the way chirp signs, because chirp's writes work on this
    // host and mine did not. The manager is bound at createContract as
    // `signerManager`; `signer` is NOT a ContractOptions key and is silently
    // dropped, and `defaultOrigin` is what the dry run prices the call for.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const keys = c.sdk.createContract(
      c.rt, DOTMAIL_KEYS, KEYS_ABI as never,
      { signerManager: c.manager, defaultOrigin: c.address } as never,
    ) as any;
