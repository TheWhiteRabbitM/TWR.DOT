...
export const TIER_KEY = ['tier.0', 'tier.1', 'tier.2'] as const;
export const TIER_CLASS = ['is-published', 'is-deployed', 'is-name'] as const;
...
const EN = {
  /* --- hero ---------------------------------------------------------- */
  'hero.title': 'Discover the .dot Ecosystem',
  'hero.sub': 'Browse apps built on the decentralized web — search, explore, and find the tools that run on .dot.',
  'hero.explore': 'Explore Apps',
  ...
  'ours.title': 'Read by dot-store',
  'ours.note':
    'dot-store hard-codes this app’s contract ABI, so these are our readings of it — our instrumentation, not something the app published, and not part of how it is ranked or ordered.',

  /* --- our copy for the four apps we read contracts for ---------------
     These are OURS, not the authors': dot-store wrote them for the apps whose
...
const IT: Record<MsgKey, string> = {
  /* --- hero ---------------------------------------------------------- */
  'hero.title': 'Scopri l\'Ecosistema .dot',
  'hero.sub': 'Esplora le app costruite sul web decentralizzato — cerca, scopri e trova gli strumenti che funzionano su .dot.',
  'hero.explore': 'Esplora le App',
  'hero.learn': 'Scopri di Più',

  /* --- featured ------------------------------------------------------ */
  'featured.title': 'App in Vetrina',
  ...
  /* --- what dot-store reads itself ---------------------------------- */
  'ours.title': 'Letto da dot-store',
  'ours.note':
    'dot-store tiene scritta a mano l’ABI del contratto di questa app, quindi queste sono nostre letture — strumentazione nostra, non qualcosa che l’app ha pubblicato, e non fanno parte di come viene ordinata o posizionata.',
...
const ES: Record<MsgKey, string> = {
  /* --- hero ---------------------------------------------------------- */
  'hero.title': 'Descubre el Ecosistema .dot',
  'hero.sub': 'Explora apps construidas en la web descentralizada — busca, descubre y encuentra las herramientas que funcionan en .dot.',
  'hero.explore': 'Explorar Apps',
  'hero.learn': 'Más Información',

  /* --- featured ------------------------------------------------------ */
  'featured.title': 'Apps Destacadas',
  ...
  /* --- what dot-store reads itself ---------------------------------- */
  'ours.title': 'Leído por dot-store',
  'ours.note':
    'dot-store lleva escrita a mano la ABI del contrato de esta app, así que estas son lecturas nuestras — instrumentación nuestra, no algo que la app haya publicado, y no forman parte de cómo se ordena ni de cómo se clasifica.',
...
const FR: Record<MsgKey, string> = {
  /* --- hero ---------------------------------------------------------- */
  'hero.title': 'Découvrez l\'Écosystème .dot',
  'hero.sub': 'Parcourez les applications construites sur le web décentralisé — cherchez, explorez et trouvez les outils qui fonctionnent sur .dot.',
  'hero.explore': 'Explorer les Apps',
  'hero.learn': 'En Savoir Plus',

  /* --- featured ------------------------------------------------------ */
  'featured.title': 'Apps en Vedette',
  ...
  /* --- what dot-store reads itself ---------------------------------- */
  'ours.title': 'Lu par dot-store',
  'ours.note':
    'dot-store code en dur l’ABI du contrat de cette app : ce sont donc nos lectures à nous — notre instrumentation, pas quelque chose que l’app a publié, et cela n’entre ni dans son classement ni dans son ordre.',
...
const STORAGE_KEY = 'dot-store.lang';
