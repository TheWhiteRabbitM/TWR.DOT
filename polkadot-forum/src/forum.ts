/**
 * Writing to the forum — the only part that needs a wallet, and only inside the
 * Polkadot app. Every write is gated to a Peoplebook mask you own (the contract
 * checks `PEOPLEBOOK.ownerOf(mask) == msg.sender`), so posting as an identity
 * means being the account it belongs to. Reads live in chain.ts and need none
 * of this.
 *
 * The signing chain is the one proven in rabbithole/src/votes.ts and
 * amazdot/src/trade.ts: createApp → app.chain.getRawClient(devnet_asset_hub) →
 * createContractRuntimeFromClient → ensureContractAccountMapped → createContract
 * → `c.method.tx(...args, WEIGHTS)`. Every SDK import is dynamic (readers
 * download none of it) and wrapped in retryImport for gateway chunk races.
 */
import type { AbiEntry } from '@parity/product-sdk-contracts';
import { FORUM_BOARD, categoryKeyOf } from './chain';
...
const WRITE_ABI: AbiEntry[] = [
  ...
  { type: 'function', name: 'like', stateMutability: 'nonpayable', inputs: [{ name: 'id', type: 'uint256' }], outputs: [] },
  { type: 'function', name: 'unlike', stateMutability: 'nonpayable', inputs: [{ name: 'id', type: 'uint256' }], outputs: [] },
  {
    type: 'function',
    name: 'edit',
    stateMutability: 'nonpayable',
  ...
  { type: 'function', name: 'remove', stateMutability: 'nonpayable', inputs: [{ name: 'id', type: 'uint256' }], outputs: [] },
];
...
function getApp(): Promise<AppLike> {
  appPromise ??= (async () => {
    ...
    const app = (await createApp({ name: 'polkadot-forum', cloudStorage: false })) as AppLike;
...
export async function connectSigner(
  ...
  const board = async () => {
    ...
    const c = createContract(runtime, FORUM_BOARD, WRITE_ABI, { defaultSigner: signer as never }) as Record<
      string,
      {
...
export async function createTopic(s: Signer, categorySlug: string, title: string, body: string): Promise<string> {
  const b = await s.board();
  return b.write('createTopic', [s.mask, categoryKeyOf(categorySlug), title, body]);
}
export async function reply(s: Signer, topicId: number, replyTo: number, body: string): Promise<string> {
  const b = await s.board();
  return b.write('reply', [s.mask, BigInt(topicId), BigInt(replyTo), body]);
}
export async function like(s: Signer, id: number): Promise<string> {
  const b = await s.board();
  return b.write('like', [BigInt(id)]);
}
export async function unlike(s: Signer, id: number): Promise<string> {
  const b = await s.board();
  return b.write('unlike', [BigInt(id)]);
}
