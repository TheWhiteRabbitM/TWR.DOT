...
export async function connectSigner(
  ...
  const board = async () => {
    ...
    const c = createContract(runtime, CANVAS, WRITE_ABI, { defaultSigner: signer as never }) as Record<
      string,
      {
...
/** Paint one pixel. Throws when the mask is still inside its cooldown, which is
 *  the contract refusing rather than this client guessing. */
export async function place(s: Signer, index: number, colour: number): Promise<string> {
  const b = await s.board();
  return b.write('place', [s.mask, index, colour]);
}
