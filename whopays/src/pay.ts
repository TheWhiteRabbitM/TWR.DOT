...
export async function connectSigner(
  ...
  const board = async () => {
    ...
    const c = createContract(runtime, WHOPAYS, WRITE_ABI, { defaultSigner: signer as never }) as Record<
      string,
      {
...
export async function openTab(s: Signer, name: string, unit: string) {
  const b = await s.board();
  return b.write('open', [s.mask, name, unit]);
}
export async function joinTab(s: Signer, id: number) {
  const b = await s.board();
  return b.write('join', [s.mask, BigInt(id)]);
}
/** Amount is in the smallest unit, so money never goes through a float. */
export async function addEntry(s: Signer, id: number, what: string, amount: bigint) {
  const b = await s.board();
  return b.write('add', [s.mask, BigInt(id), what, amount, 0]);
}
