...
const WRITE_ABI: AbiEntry[] = [
  {
    type: 'function', name: 'enrol', stateMutability: 'nonpayable',
    inputs: [
  ...
  {
    type: 'function', name: 'cast', stateMutability: 'nonpayable',
    inputs: [
      { name: 'id', type: 'uint256' },
      { name: 'option', type: 'uint8' },
      { name: 'c0', type: 'uint256' },
      { name: 's', type: 'uint256[]' },
      { name: 'keyImage', type: 'uint256[2]' },
    ],
    outputs: [],
  },
];
...
export async function connectSigner(
  ...
  const board = async () => {
    ...
    const c = createContract(runtime, BALLOT, WRITE_ABI, { defaultSigner: signer as never }) as Record<
      string,
      {
...
/** The one moment identity is used: a mask joins the ring. */
export async function enrolKey(s: Signer, pollId: number, pub: readonly [bigint, bigint]) {
  const b = await s.board();
  return b.write('enrol', [BigInt(pollId), s.mask, [pub[0], pub[1]]]);
}

/** And the one moment it is not: a ballot carries a proof, not a name. */
export async function castBallot(
  ...
  c0: bigint,
  sig: bigint[],
  keyImage: readonly [bigint, bigint],
) {
  const b = await s.board();
  return b.write('cast', [BigInt(pollId), option, c0, sig, [keyImage[0], keyImage[1]]]);
}
