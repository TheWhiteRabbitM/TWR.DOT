...
/// @title PeoplebookMasks2 — one unforgeable mask per account, with a name
...
///         exists to close.
/// @custom:cdm @thebutton/peoplebookmasks2
contract PeoplebookMasks2 {
    // --------------------------------------------------------------- errors
    ...
    event ProfileSet(uint256 indexed id, string displayName, string telegram, string x, string bio);
    ...
    /// @dev `displayName` is free text you choose, like the name on X — it is NOT
    ///      proof of anything, which is exactly why the .dot tick stays separate.
    struct Profile { string displayName; string telegram; string x; string bio; }
    ...
    function setProfile(string calldata displayName, string calldata telegram, string calldata x, string calldata bio) external {
        uint256 id = maskOf[msg.sender];
        if (id == 0) revert NoMask();
        if (bytes(displayName).length > 40 || bytes(telegram).length > 32 || bytes(x).length > 32 || bytes(bio).length > 160) revert BadProfile();
        if (!_jsonSafe(bytes(displayName)) || !_jsonSafe(bytes(telegram)) || !_jsonSafe(bytes(x)) || !_jsonSafe(bytes(bio))) revert BadProfile();
        _profile[id] = Profile(displayName, telegram, x, bio);
        emit ProfileSet(id, displayName, telegram, x, bio);
    }

    /// @notice The whole profile, display name included — a name you can write but
    ///         not read back would be no name at all.
    function profileOf(uint256 id)
        external
        view
        returns (string memory displayName, string memory telegram, string memory x, string memory bio)
    {
        Profile storage p = _profile[id];
        return (p.displayName, p.telegram, p.x, p.bio);
    }
    ...
    function tokenURI(uint256 id) external view returns (string memory) {
        ...
        string memory image = string.concat(
            "data:image/svg+xml;base64,", B64.encode(bytes(_svg(o)))
        );
        ...
        return string.concat("data:application/json;base64,", B64.encode(bytes(json)));
    }
